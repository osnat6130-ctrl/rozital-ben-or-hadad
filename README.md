...
### ‼️ אם האתר מפסיק להתעדכן אחרי שמירה בפאנל

התסמין: רוזיטל שומרת, נוצר commit בריפו, והאתר לא משתנה. זה **לא** תקלה
בפאנל, וכל השינויים שלה בטוחים בריפו - פשוט אף אחד לא בנה את האתר.

מה לבדוק, בסדר הזה:

1. **Cloudflare Pages > rozital-ben-or-hadad > Deployments.** אם הבנייה
   האחרונה מלפני כמה ימים, ואין בנייה לכל commit של הפאנל - הטריגר
   מגיטהאב לא עובד.
2. אם מופיע שם הפס **"This project is disconnected from your Git
   account"** - זו הסיבה.
3. הבעיה האמיתית היא ב-GitHub: **Settings > Applications > Cloudflare
   Workers and Pages > Repository access.** ההרשאה שם היא "Only select
   repositories".

   ‼️ **הבורר הזה מחליף ולא מוסיף.** כשמחברים פרויקט חדש בקלאודפלייר
   (למשל אתר של לקוחה אחרת), הבחירה החדשה יכולה לדרוס את הריפו הזה -
   ואז האתר החי של רוזיטל מפסיק להתעדכן **בשקט**, בלי שגיאה בפאנל ובלי
   התראה. זה קרה בפועל ב-14.9.2026: ברשימה נשאר רק `or-vehomer`,
   והאתר היה קפוא 4 ימים.

   התיקון: להוסיף את `osnat6130-ctrl/rozital-ben-or-hadad` חזרה
   לרשימה ולשמור. הבנייה הבאה תעלה את כל מה שהצטבר.
4. אחרי התיקון צריך push כלשהו כדי לעורר בנייה - חיבור מחדש לא בונה
   לבד את מה שהתפספס.

**מניעה:** לשקול "All repositories" בהרשאת האפליקציה, כדי שהוספת פרויקט
לא תוכל לנתק פרויקט קיים.
